...
### Image / CVE source of truth (strict gating — shipped)
- **CVE IDs and CVE↔image pairs come only from Excel/JSON structured attachment facts** (`cve_image_facts`, including Sonatype/Nexus IQ JSON). Description text and PDF free-text do not create findings rows or PLAT image slots (description mentions are stored as `description_cves` for reference only).
